...
// solving exam (resolviendo examen)

// Get references to HTML elements (tomando referenaicas desde los elementos html)
const searchButton = document.getElementById('searchButton');

// Add event listener to the search button (agregando los eventos de lista del boton buscar)
searchButton.addEventListener('click', function () {
    // Get filter values (obteniendo valores filtrados)
    const fromDate = new Date(document.getElementById('fromDate').value);
    const toDate = new Date(document.getElementById('toDate').value);
    const rating = Number(document.getElementById('rate').value);
    const userId = document.getElementById('userId').value;

    // Call the filterMovies function with the provided filter values (llamado a la funcion filterMovies con el provisto filtro de valores)
    const filteredResults = filterMovies(MOVIES, USERS, fromDate, toDate, rate, userId);

    // Display the filtered results (Mostrar los resultados filtrados)
    displayResults(filteredResults);
});

// Function to display the filtered results (funcion para mostrar el resultado de lo filtrado )
function displayResults(filteredResults) {
    ...
    filteredResults.forEach(result => {
      ...
      <td>${result.rate}</td>
    `;
        filteredResultsContainer.appendChild(row);
    });
}

// Function to filter movies (funcion para filtrar peliculas)


function filterMovies(MOVIES, USERS, fromDate, toDate, rating, userId) {
    return MOVIES.reduce((filteredResults, movie) => {
        const movieDate = new Date(movie.watched.replace(' ', 'T'));

        if (
            (fromDate && movieDate <= fromDate) ||
            (toDate && movieDate >= toDate) ||
            (rating && movie.rate !== rate) ||
            (userId && movie.userId !== userId)
        ) {
